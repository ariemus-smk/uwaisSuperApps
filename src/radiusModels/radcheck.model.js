/**
 * Radcheck model for RADIUS DB.
 * Provides CRUD operations for the `radcheck` table.
 * Stores per-user check attributes (PPPoE credentials).
 */

const { radiusPool } = require('../config/database');

/**
 * Find all check attributes for a username.
 * @param {string} username - PPPoE username
 * @returns {Promise<Array>} List of radcheck records
 */
 ...
 * @param {string} username - PPPoE username
 * @param {string} attribute - Attribute name (e.g., 'Cleartext-Password')
 * @returns {Promise<object|null>} Record or null
 */
async function findByUsernameAndAttribute(username, attribute) {
  const [rows] = await radiusPool.execute(
    'SELECT * FROM radcheck WHERE username = ? AND attribute = ? LIMIT 1',
    [username, attribute]
  );
...
async function findById(id) {
  const [rows] = await radiusPool.execute(
    'SELECT * FROM radcheck WHERE id = ? LIMIT 1',
    [id]
  );
  return rows.length > 0 ? rows[0] : null;
}
...
/**
 * List all radcheck records with optional filters.
...
async function findAll(filters = {}) {
  ...
  let countQuery = 'SELECT COUNT(*) as total FROM radcheck WHERE 1=1';
  let dataQuery = 'SELECT * FROM radcheck WHERE 1=1';
  const params = [];
...
/**
 * Create a new radcheck entry.
 * @param {object} data
 * @param {string} data.username - PPPoE username
 * @param {string} data.attribute - Attribute name (e.g., 'Cleartext-Password')
...
async function update(id, data) {
  ...
  const [result] = await radiusPool.execute(
    `UPDATE radcheck SET ${setClauses.join(', ')} WHERE id = ?`,
    params
  );
...
/**
 * Delete all radcheck entries for a username.
