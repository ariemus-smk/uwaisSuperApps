...
- [ ] 2. Authentication and authorization middleware
  ...
  - [~] 2.2 Implement RBAC middleware with permission matrix
  ...
  - [~] 2.3 Write property test for RBAC permission enforcement
    - **Property 11: RBAC Permission Enforcement**
    - **Validates: Requirements 31.3**

  - [~] 2.4 Implement Branch scoping middleware
  ...
  - [~] 2.5 Implement auth controller and routes
  ...
  - [~] 2.6 Write unit tests for auth middleware and service
...
- [ ] 4. Core utility modules
  - [~] 4.1 Implement Indonesian phone number validator
  ...
  - [~] 4.3 Implement prorata billing calculator
  ...
  - [~] 4.4 Write property test for prorata calculation
    - **Property 4: Prorata Billing Calculation**
    - **Validates: Requirements 5.1**

  - [~] 4.5 Implement PPPoE credential generator
...
- [ ] 5. Branch and user management modules
  - [~] 5.1 Implement Branch model, service, controller, and routes
  ...
  - [~] 5.2 Implement User management model, service, controller, and routes
  ...
  - [~] 5.3 Write unit tests for branch and user services
    - Test branch deactivation logic
    - Test Mitra/Merchant account creation with specific fields
    - _Requirements: 33.3, 32.2, 32.3_

- [ ] 6. Customer lifecycle and database module
  - [~] 6.1 Implement Customer model with lifecycle state machine
  ...
  - [~] 6.2 Write property test for customer lifecycle state machine
    - **Property 1: Customer Lifecycle State Machine**
    - **Validates: Requirements 1.3, 1.5**

  - [~] 6.3 Implement Customer service with validation and branch scoping
  ...
  - [~] 6.4 Implement Customer controller and routes
  ...
  - [~] 6.5 Write unit tests for customer service
...
- [ ] 7. Service package management module
  - [~] 7.1 Implement Package model, service, controller, and routes
  ...
  - [~] 7.2 Write property test for QoS parameter constraints
    - **Property 3: QoS Parameter Constraints**
    - **Validates: Requirements 4.2, 4.3**

  - [~] 7.3 Write unit tests for package service
...
- [ ] 8. Subscription and RADIUS provisioning module
  - [~] 8.1 Implement RADIUS models for RADIUS DB
  ...
  - [~] 8.2 Implement RADIUS service for provisioning
  ...
  - [~] 8.3 Implement Subscription model, service, controller, and routes
  ...
  - [~] 8.4 Write unit tests for subscription and RADIUS service
...
- [ ] 10. Billing and invoice module
  - [~] 10.1 Implement Invoice model and billing service
  ...
  - [~] 10.2 Write property test for invoice total with PPN
    - **Property 5: Invoice Total with PPN**
    - **Validates: Requirements 6.2**

  - [~] 10.3 Write property test for down payment deduction
    - **Property 13: Down Payment Deduction**
    - **Validates: Requirements 46.2, 46.3**

  - [~] 10.4 Implement Billing controller and routes
  ...
  - [~] 10.5 Implement Down Payment model and service
  ...
  - [~] 10.6 Write unit tests for billing service
...
- [ ] 11. Payment processing module
  - [~] 11.1 Implement Tripay integration service
  ...
  - [~] 11.2 Implement Payment model and payment service
  ...
  - [~] 11.3 Implement Mitra payment service
  ...
  - [~] 11.7 Write unit tests for payment services
...
- [ ] 13. CoA/POD engine module
  - [~] 13.1 Implement CoA packet builder utility
    - Create `src/utils/coaPacket.js` with RADIUS CoA-Request and POD packet construction
    - Build UDP packets with proper RADIUS attributes (User-Name, Mikrotik-Rate-Limit, etc.)
    - Include RADIUS authenticator calculation using shared secret
    - _Requirements: 13.1, 13.2_

  - [~] 13.2 Implement CoA service with retry logic
    - Create `src/services/coa.service.js` with sendCoA, sendPOD functions
    - Implement UDP client sending to NAS port 3799
    - Wait for CoA-ACK/CoA-NAK response with timeout
    - Implement retry logic: 3 retries with exponential backoff (1s, 2s, 4s)
    - Log all operations to coa_logs table
    - _Requirements: 13.1, 13.2, 13.3, 13.4, 13.5, 13.6_

  - [~] 13.3 Write property test for retry logic
    - **Property 7: Retry Logic with Maximum Attempts**
    - **Validates: Requirements 7.5, 13.4, 30.4**

  - [~] 13.4 Implement CoA controller and routes
  ...
  - [~] 13.5 Write unit tests for CoA service
    - Test packet construction
    - Test response parsing (ACK, NAK, Timeout)
    - Test retry state machine
    - _Requirements: 13.3, 13.4, 13.5_

- [ ] 14. NAS management module
  - [~] 14.1 Implement NAS model and service
    - Create `src/models/nas.model.js` with CRUD and status tracking
    - Create `src/services/nas.service.js` with register (auto-create 4 VPN accounts), generateScript, testConnectivity
  ...
  - [~] 14.2 Implement NAS monitoring service
  ...
  - [~] 14.3 Implement NAS controller and routes
  ...
  - [~] 14.4 Write unit tests for NAS service
    - Test VPN account generation
    - Test Mikrotik script generation
    - Test connectivity test logic
    - _Requirements: 12.2, 12.3, 12.5_

- [ ] 15. Infrastructure module (OLT, ODP, Coverage)
  - [~] 15.1 Implement OLT model, service, and routes
  ...
  - [~] 15.2 Implement ODP model, service, and coverage check
  ...
  - [~] 15.3 Write property test for ODP capacity exclusion
    - **Property 10: ODP Capacity Exclusion**
    - **Validates: Requirements 29.5**

  - [~] 15.4 Implement Infrastructure controller and routes
  ...
  - [~] 15.5 Write unit tests for coverage service
...
- [ ] 17. Customer activation and package change flows
  - [~] 17.1 Implement customer activation flow service
  ...
  - [~] 17.2 Implement package change request service
  ...
  - [~] 17.3 Write property test for package change rate limiting
    - **Property 8: Package Change Rate Limiting**
    - **Validates: Requirements 17.2**

  - [~] 17.4 Implement package change controller and routes
  ...
  - [~] 17.5 Write unit tests for activation and package change
...
- [ ] 18. Asset and inventory management module
  - [~] 18.1 Implement Asset inbound model and service
  ...
  - [~] 18.2 Implement Asset outbound and installation tracking
  ...
  - [~] 18.3 Implement Tool lending service
  ...
  - [~] 18.4 Implement Inter-branch transfer service
  ...
  - [~] 18.5 Implement Direct sales and stock opname
  ...
  - [~] 18.6 Implement Asset controller and routes
  ...
  - [~] 18.7 Write unit tests for asset service
...
- [ ] 19. Helpdesk ticketing module
  - [~] 19.1 Implement Ticket model and service
  ...
  - [~] 19.2 Implement Technician dispatch and multi-ticket assignment
  ...
  - [~] 19.3 Implement Remote troubleshooting integration
  ...
  - [~] 19.4 Implement Ticket resolution and KPI tracking
  ...
  - [~] 19.5 Implement Overtime model and service
  ...
  - [~] 19.6 Implement Ticket controller and routes
  ...
  - [~] 19.7 Write unit tests for ticket service
    - Test priority classification logic
    - Test resolution time calculation
    - Test overtime workflow
    - _Requirements: 24.2, 27.1, 39.3_

- [ ] 20. Notification module (WhatsApp queue)
  - [~] 20.1 Implement Notification model and queue service
  ...
  - [~] 20.2 Implement Notification controller and routes
  ...
  - [~] 20.3 Integrate notifications into billing and payment flows
  ...
  - [~] 20.4 Write unit tests for notification service
    - Test queue processing
    - Test retry logic
    - Test channel selection based on subscription age
    - _Requirements: 30.4, 6.6_

- [ ] 21. ACS/TR-069 integration module
  - [~] 21.1 Implement ACS service and routes
  ...
  - [~] 21.2 Write unit tests for ACS service
...
- [ ] 23. Scheduled jobs module
  - [~] 23.1 Implement job scheduler infrastructure
  ...
  - [~] 23.2 Implement billing generation job
  ...
  - [~] 23.3 Implement auto-isolir job
  ...
  - [~] 23.4 Implement NAS health polling job
  ...
  - [~] 23.5 Implement FUP enforcement job
  ...
  - [~] 23.6 Write property test for FUP threshold enforcement
    - **Property 14: FUP Threshold Enforcement**
    - **Validates: Requirements 41.2**

  - [~] 23.7 Implement FUP reset job
  ...
  - [~] 23.8 Implement KPI calculation job
  ...
  - [~] 23.9 Implement notification broadcast job
  ...
  - [~] 23.10 Implement Scheduler controller and routes
  ...
  - [~] 23.11 Write unit tests for scheduled jobs
...
- [ ] 24. Bill waiver and extended isolir logic
  - [~] 24.1 Implement bill waiver service
  ...
  - [~] 24.2 Write unit tests for bill waiver
...
- [ ] 26. KPI, payroll, and overtime module
  - [~] 26.1 Implement KPI model, service, and routes
  ...
  - [~] 26.2 Implement Payroll model, service, and routes
  ...
  - [~] 26.3 Write unit tests for KPI and payroll services
    - Test KPI score calculation
    - Test payroll consolidation
    - Test approval workflow
    - _Requirements: 38.1, 40.1, 40.2_

- [ ] 27. CAPEX and expansion budgeting module
  - [~] 27.1 Implement CAPEX model, service, and routes
  ...
  - [~] 27.2 Write unit tests for CAPEX service
...
- [ ] 28. Reporting and export module
  - [~] 28.1 Implement Komdigi regulatory reports
  ...
  - [~] 28.2 Implement financial reports
  ...
  - [~] 28.3 Implement customer growth reports
  ...
  - [~] 28.4 Write property test for net growth calculation
    - **Property 15: Net Growth Calculation**
    - **Validates: Requirements 36.1**

  - [~] 28.5 Implement Report controller and routes
  ...
  - [~] 28.6 Write unit tests for report services
...
- [ ] 29. Customer self-service module
  - [~] 29.1 Implement Self-service controller and routes
  ...
  - [~] 29.2 Write unit tests for self-service
    - Test data isolation (own data only)
    - Test WiFi change triggers ACS
    - _Requirements: 43.5, 43.2_

- [ ] 30. Technician work journal module
  - [~] 30.1 Implement Work journal model, service, and routes
  ...
  - [~] 30.2 Write unit tests for work journal
...
- [ ] 32. Route aggregation and final wiring
  - [~] 32.1 Wire all routes into Express app
  ...
  - [~] 32.2 Implement system settings model and service
  ...
  - [~] 32.3 Create seed data and initial migration runner
  ...
  - [~] 32.4 Write integration tests for critical flows
